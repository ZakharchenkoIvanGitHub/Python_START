...
class MyTelBot:
    ...
    @staticmethod
    def send_message(chat_id, text, url, token):
...
class Chat:
    ...
    def processing_message(self, text):
        ...
        if self.phone_book:  # Работа в телефонной книге
            if self.add_rec:
                ...
                for card in data:
                    MyTelBot.send_message(self.chat_id, tel_book.create_card(card), self.url, self.token)
                ...
                match text:
                    ...
                    case "Поиск_по_фамилии":
                        ...
                        MyTelBot.send_message(self.chat_id, "Введите фамилию", self.url, self.token)
                    case "Выход":
                ...
                if text == "Выход":
                    ...
                    try:
                        ...
                        MyTelBot.send_bottom(self.chat_id,
                                             "Неверный формат ввода данных, попробуйте еще раз  или нажмите Выход",
                                             self.menu_polynom,
                                             self.url, self.token)

                    else:
                        MyTelBot.send_message(self.chat_id, f'Сумма полиномов {self.res_polynom}', self.url, self.token)
                        MyTelBot.send_bottom(self.chat_id, "Добавить еще или нажмите Выход", self.menu_polynom,
                                             self.url, self.token)
            else:
                self.res_polynom = text
                MyTelBot.send_message(self.chat_id, 'Введите еще полином ', self.url, self.token)
        else:
            match text:
                case "/start":
                    MyTelBot.send_message(self.chat_id, f'Привет {self.first_name}', self.url, self.token)
                    MyTelBot.send_bottom(self.chat_id, "Выберете пункт меню", self.operation, self.url, self.token)
                ...
                case "Сложение_многочленов":
                    ...
                    MyTelBot.send_message(self.chat_id, "Введите полином формата 9x^5+7x^4+7x^3+9x^2+6x+17=0", self.url,
                                          self.token)
    ...
    def add_record(self, text=None):
        ...
        if self.add_first_name:
            ...
            MyTelBot.send_message(self.chat_id, 'Введите номера телефонов через пробел', self.url, self.token)
            self.add_tel = True
        elif self.add_tel:
            self.card["phone_number"] = text.split()
            self.add_tel = False
            MyTelBot.send_message(self.chat_id, 'Введите дату рождения', self.url, self.token)
            self.add_birthday = True
        elif self.add_birthday:
            self.card["birthday"] = text
            self.add_birthday = False
            MyTelBot.send_message(self.chat_id, 'Введите место работы', self.url, self.token)
            ...
            if self.edit_rec:
                ...
                MyTelBot.send_bottom(self.chat_id, "Запись изменена\nВыберете пункт меню", self.menu_phone_book,
                                     self.url, self.token)
                self.edit_rec = False
            else:
                tel_book.add_data(self.card)
                MyTelBot.send_bottom(self.chat_id, "Запись добавлена\nВыберете пункт меню", self.menu_phone_book,
                                     self.url, self.token)
        else:
            MyTelBot.send_message(self.chat_id, 'Введите имя', self.url, self.token)
    ...
    def edit_record(self, text=None):
        ...
        if self.get_id:
            ...
            if self.id_rec:
                MyTelBot.send_message(self.chat_id, tel_book.create_card(tel_book.get_data_id(self.id_rec)), self.url,
                                      self.token)
                self.get_id = False
                self.add_rec = True
                self.add_record()
                ...
                MyTelBot.send_message(self.chat_id, "Неверное значение. Введите ID записи", self.url, self.token)

        else:
            MyTelBot.send_message(self.chat_id, "Введите ID записи", self.url, self.token)
            self.get_id = True
